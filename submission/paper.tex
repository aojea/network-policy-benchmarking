...
matched 4-tier kube-network-policies bidirectional mesh sweep across 7, 700,
3,500, and 35,000 identities at 500 QPS. Under ReplicaSet-paced deployment,
kube-network-policies maintains low post-scheduling latency (5.33 s P99 at
700 identities and 2.79 s P99 at 3,500 identities) and flat worker CPU
(0.45--0.52 mean cores), while under an unbuffered 35,000 raw-Pod burst it
admits all 35,000 Pods with zero stranded workloads (experiencing only
physical worker queue drain of 120.0 s P50 / 421.4 s P99 where Cilium is
blocked by per-endpoint map capacity and its 65,280 identity ceiling).
We also analyze the connection-processing and fail-open trade-offs of
deferred evaluation and enumerate the remaining packet-level measurements
needed to complete the comparison.
\end{abstract}
...
Fifth, our matched 4-tier \sys{} bidirectional mesh sweep across 7, 700,
3,500, and 35,000 identities confirms that deferred userspace evaluation
decouples workload admission from peer identity cardinality: post-scheduling
latency remains low under ReplicaSet-paced deployment (5.33 s P99 at 700
identities and 2.79 s P99 at 3,500 identities) with flat worker CPU
(0.45--0.52 mean cores), and under an unbuffered 35,000 raw-Pod burst, \sys{}
admits 100\% of all 35,000 Pods with zero stranded workloads. None
...
the newly executed 4-tier bidirectional mesh sweep. Section~\ref{sec:gaps}
enumerates the packet-level and microbenchmark measurements that the
artifacts still do not contain, such as sustained denied-traffic overload
and isolated NRI ablations. Making those limits explicit lets the
...
Within the ReplicaSet-paced tiers, increasing intended identities does not
produce an increasing post-scheduling tail for either engine. In Cilium's
open-gateway case, P99 decreases from 5.930 s to 1.990 s between the
endpoint tiers (hub-and-spoke: 6.370 s to 2.033 s). In the bidirectional
mesh topology, \sys{} post-scheduling P99 drops from 5.331 s at 700
identities to 2.791 s at 3,500 identities (P50: 3.273 s and 2.234 s).
At Tier 1 (7 ReplicaSets of 5,000 Pods), simultaneous controller emission
produces a large initial scheduling burst that elevates \sys{} post-scheduling
latency to 38.946 s P50 / 151.479 s P99. At Tier 4 (35,000 identities),
where Cilium is excluded by per-endpoint map capacity ($2N=70{,}000 > 65{,}536$),
\sys{} executes the full unbuffered raw-Pod burst to 100\% completion
(35,000 Running, 0 stranded), experiencing a 120.003 s P50 / 421.410 s P99
post-scheduling tail driven by physical worker-node kubelet queue drain.
...
35,000 & 1/Raw & 1.990 & 2.033 & Excl. & 421.410$^{*}$\\
\bottomrule
\end{tabular}
\caption{Per-run archived startup percentiles (schedule-to-run P99). All
ReplicaSet-paced suites have zero JUnit failures. $^{*}$Tier 4 \sys{} Mesh
uses 35,000 unbuffered raw Pods (100\% Running, 0 stranded; 2 JUnit SLO
threshold alerts on the 5m0s P90 pod startup target due to 357.7 s queue drain).}
\label{tab:identity}
\end{table}
...
\addlegendentry{Cilium Mesh: executed tiers}
\addplot+[mark=diamond*,dashdotted,thick] coordinates {(700,5.331) (3500,2.791)};
...
quantiles for ReplicaSet-paced tiers (connecting lines are visual guides;
\sys{} Tier 1 5,000-ppr burst at 151.5 s and Tier 4 unbuffered raw-Pod queue
drain at 421.4 s are omitted from the 0--8 s vertical scale). Right: conditional
permission-state model for eager per-endpoint compilation. In \texttt{agentic-cilium.k8s.local},
\texttt{--bpf-policy-map-max} was raised to 65,536, permitting unidirectional 35k
tiers while excluding bidirectional mesh at 35k ($2I=70{,}000 > 65{,}536$).}
\label{fig:scaling}
\end{figure*}
...
half of the per-second samples were zero. $^{\ddagger}$Peak scheduling rate reached 841 Pods/s (826.6 P99).}
\label{tab:resources}
\end{table}
...
The mesh tiers provide direct comparative evidence across both engines.
For Cilium, post-scheduling medians remain approximately 1.41--1.42 s
across the three executed tiers, while end-to-end P99s are 33.534, 28.739,
and 21.730 s (initial setup plus wait durations: 192.109, 137.947, and
109.718 s). For \sys{}, under ReplicaSet-paced injection at 700 and 3,500
identities (50 and 10 Pods per ReplicaSet), post-scheduling latency is
3.273 s P50 / 5.331 s P99 and 2.234 s P50 / 2.791 s P99 (end-to-end
\texttt{pod\_startup} P99: 5.553 s and 2.918 s). These measurements confirm that
evaluating policy semantics in userspace on attempted connections decouples
container admission from peer identity cardinality.

Comparing the generator modes across the 4-tier \sys{} sweep also reveals
how controller pacing interacts with worker-node queues. At Tier 1
(7 ReplicaSets of 5,000 Pods), simultaneous emission of 5,000 pods per
controller creates an unpaced scheduling wave that drives \sys{}
post-scheduling P99 to 151.479 s, whereas dividing the same 35,000 Pods
across 700 or 3,500 ReplicaSets paces controller emission and lowers
post-scheduling P99 to 5.331 s and 2.791 s. At Tier 4 (35,000 identities
submitted as unbuffered raw Pods at 500 QPS), the scheduler binds all
35,000 Pods in under 50 seconds (peaking at 841 Pods/s). Because roughly
48 Pods arrive simultaneously at each worker node, sequential container
creation in kubelet produces a 120.003 s P50 / 421.410 s P99 queue-drain
tail. Crucially, unlike Cilium---which aborted with 5,694 stranded Pods in
its raw-Pod burst and was blocked from Tier 4 mesh by per-endpoint map limits---\sys{}
admitted 100\% of all 35,000 raw Pods with zero stranded workloads.
...
We label it an \emph{analytical exclusion}. In the tested cluster
(\texttt{agentic-cilium.k8s.local}), the \texttt{cilium-config} ConfigMap
explicitly raised \texttt{bpf-policy-map-max} from its documented default
of 16,384 to its upper 16-bit limit of 65,536~\cite{ciliumagentref}.
This explains why the unidirectional 35,000-identity tiers ($I=35{,}000 < 65{,}536$)
completed with 35,000 Pods Running on the gateway node, whereas the
bidirectional mesh tier ($2I=70{,}000 > 65{,}536$) exceeded the maximum
allocatable map size on every sandbox endpoint. The agent also exposes an
...
require this permission cross-product, and our 4-tier \sys{} mesh sweep
confirms that \sys{} admits all 35,000 Pods at Tier 4 without map overflow.
Instead, \sys{} capacity depends on metadata storage, conntrack table sizing,
and userspace decision rate under active connection establishment.
...
\label{sec:gaps}
While our newly executed 4-tier \sys{} bidirectional mesh sweep resolves
the primary macro-benchmark gap---confirming that \sys{} scales across
7, 700, 3,500, and 35,000 identities without map overflow or stranded
workloads---a scan of both repositories identifies seven remaining
packet-level and diagnostic measurements required to complete the
evaluation (Table~\ref{tab:gaps}). Each row names the missing evidence,
the claim it would support or refute, and the cheapest experiment that
would produce it.
...
\addlinespace
Policy-map occupancy dumps (\texttt{cilium-dbg bpf policy get}) & empirical occupancy vs. $2I$ model & dump policy maps on one node per tier\\
\addlinespace
...
\addlinespace
NRI on/off with identical binaries & isolated startup-metadata benefit & rerun baseline QPS 500 and tuning bundle 1 with one pinned image\\
\addlinespace
Effective binary digests and flags per run & attribution to inspected code & record image digests and args in \texttt{cl2-metadata}\\
\addlinespace
Raw logs for the v1.18.6 and raw-Pod aborts & causal diagnosis of both failures & retain agent and operator logs on abort\\
\bottomrule
\end{tabular}
\caption{Packet-level and diagnostic evidence still absent from the benchmark
and implementation repositories after incorporating the 4-tier \sys{} mesh sweep.}
\label{tab:gaps}
\end{table}
...
Our 4-tier bidirectional mesh sweep confirms the macro-scale
admission and capacity benefits, while leaving the fine-grained packet
overload boundary to the microbenchmarks of Table~\ref{tab:gaps}.
...
connection decisions in the kernel. NRI removes the asynchronous
API round trip from local Pod-address discovery without pretending
to eliminate all distributed-state dependencies.

Our empirical evaluation across 35,000 Pods on 720 worker nodes
demonstrates both sides of this trade-off. Completed Cilium sweeps
retain low post-scheduling latency and flat worker CPU within their
feasible region, but halve achieved throughput under 35,000 ReplicaSets,
require analytical exclusion at 35,000 bidirectional mesh identities
due to the 65,536 BPF map ceiling, and suffer pod stranding under
unbuffered raw-Pod bursts. In contrast, our matched 4-tier \sys{}
bidirectional mesh sweep confirms that deferred evaluation achieves
2.79--5.33 s post-scheduling P99 at 700--3,500 identities and admits
all 35,000 raw Pods at Tier 4 with zero stranded workloads. Completing
the systems comparison now requires the packet-level measurements of
Table~\ref{tab:gaps}, specifically quantifying NFQUEUE verdict latency
under sustained denied-traffic overload and isolating the contribution
of NRI metadata delivery.
...
The matched \sys{} (Kindnet) bidirectional mesh reports are in
...
\path{tier4-35000-identities-rawpods}.
...
\path{artifacts_pods/kindnet/mesh-sweep/}, providing the complete 4-tier
\sys{} bidirectional mesh sweep across 7, 700, 3,500, and 35,000 identities.
