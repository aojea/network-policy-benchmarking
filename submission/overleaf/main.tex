...
identity sweeps retain low post-scheduling latency, while a dense-mesh tier
was excluded because its estimated per-endpoint permission state exceeded
the stated map capacity. A separate direct-burst experiment documents 5,694
stranded Pods. These observations motivate distinguishing latency, capacity,
and admission failure, rather than treating identity cardinality as a single
scaling variable. We identify the conditions under which deferred evaluation
could reduce eager policy state, the new connection-processing costs it
introduces, and the measurements needed to establish its comparative benefit.
\end{abstract}
...
Our evidence distinguishes three observations. First, the completed
...
1.99-second P99. Second, under the benchmark's per-endpoint expansion model,
a bidirectional mesh with 35,000 identities requires approximately 70,000
peer-policy entries per endpoint. The experiment producer excluded that
tier on predicted capacity grounds; it was not executed. Third, a separate
raw-Pod burst aborted with 29,306 Pods Running and 5,694 stranded, attributed
in the outcome record to endpoint throttling and cumulative identity
allocation. None of these observations should be substituted for another.
...
mechanisms; presents a conditional model of eager permission state versus
connection-driven work; and analyzes an archived large-cluster benchmark
collection, including its limitations and incomplete outcomes. It does
\emph{not} yet establish superior identity scaling in a matched \sys{}
comparison, or quantify NFQUEUE's connection-admission overhead. Making
those limits explicit lets the evaluation test the design rather than
...
The inspected command-line interface defaults to fail-open. A
security-oriented deployment cannot infer fail-closed behavior from the
architecture alone: it must record the effective configuration and
validate bootstrap, overload, agent loss, and upgrades. Furthermore, the
inspected implementation logs an initial nftables synchronization error
without blocking startup, and its fail-closed graceful-shutdown path
removes its nftables rules. Those transitions require a deployment-level
isolation contract or implementation changes before claiming protection
...
connections. Neither $\lambda_q$ nor $\mu$ has been established by the
startup reports analyzed here.
...
application-visible connection setup. The implementation also has
...
The benchmark image digests have not been matched to the inspected
source revision. We therefore do not attribute their latency differences
solely to the code described here.
...
\addlegendentry{Guard fallback: 16,384}
\addplot+[mark=none,dotted,black,thick] coordinates {(5,65536) (50000,65536)};
\addlegendentry{Stated limit: 65,536}
\end{axis}
\end{tikzpicture}
\end{minipage}
...
excluded and has no latency point. The stated map limit requires
version-specific verification; it is not a universal eBPF limit.}
\label{fig:scaling}
\end{figure*}
...
with each tier. In particular, one Pod per ReplicaSet is not equivalent
to a few large ReplicaSets at the same object-operation rate. A
controlled identity-turnover test should hold these quantities fixed
and independently vary label sharing and label freshness.
...
artifacts. We therefore plot no inferred constant Kindnet line. An
...
We label it an \emph{analytical exclusion}. The exact per-identity
representation and configurable maximum must be verified for the tested
Cilium binary. An alternative representation could move this boundary.
...
before the queue becomes the bottleneck. This can test the displaced
cost without repeating a full 720-node sweep.
...
QPS 50 and 100, both families have JUnit failures despite producing
startup reports; these rows must not be described as wholly successful
experiments.
...
tiers with JUnit failures in both configurations. These are complete-stack
observations, not an isolated NFQUEUE versus eBPF comparison.}
\label{tab:qps}
\end{table}
...
The collection's limitations affect the conclusions directly. Exact
image provenance and effective settings remain incomplete. Most
...
individual Pod trajectories. Some attempts lack final reports, and
the counts of failed JUnit elements are not counts of failed Pods.
CPU maxima based on one-minute rates are not instantaneous peaks.
Finally, no matched Kindnet high-identity sweep or connection-rate
evaluation is present. We use these artifacts to locate the questions
the architecture must answer, not to manufacture a comparative scaling
curve from missing measurements.
...
benefit~\cite{k8s114659}. IPTracker is implemented future work in
this paper's evaluation scope, not a feature credited with the
reported startup results.
...
post-scheduling latency; the producer excludes a dense-mesh tier
on predicted permission-state capacity grounds; and a separate
direct burst documents an admission failure. Together they motivate
a comparison based on feasible state and attempted communication,
not a universal ranking of datapath technologies. Establishing the
benefit of deferred policy evaluation now requires a matched
identity-turnover comparison and explicit measurements of connection
...
\path{pkg/dataplane/controller.go}, \path{pkg/cmd/cmd.go},
\path{pkg/podinfo/nri/resolver.go}, and
\path{pkg/ipcache/{client,bbolt,lru}.go}, inspected at
\texttt{9984dcd}. Source behavior has not been equated to the
benchmark binaries. \path{submission/evidence-audit.md} records
